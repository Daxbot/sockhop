...
class SockhopClient extends EventEmitter{
     ...
     */
    _perform_auto_reconnect(force=false){

        // If auto reconnect has been disabled, we can disregard
        if(!this.auto_reconnect && !force) return;
    ...
    async resolve_on_connect({ timeout=null }={}) {
        ...
        return new Promise((resolve,reject) => {
            ...
            this._auto_reconnect_timer = setInterval(()=>this._perform_auto_reconnect(true), this._auto_reconnect_interval);
        }).catch((err) => {
            clearInterval(this._auto_reconnect_timer);
            throw err;
        });
