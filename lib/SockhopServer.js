...
class SockhopServer extends EventEmitter {
        ...
        this.server.on('connection', async (sock) => {
            ...
            sock.on("end",()=>{
                ...
                if ( sock._binary_mode.rx ) {
                    sock.object_buffer.buf2obj(buf).forEach(({ name, obj })=>{
    ...
    _on_recieved_object(type, data, { callback_response, callback_request, callback_id }={}, sock, sess) {
        ...
        default:
            ...
            if(callback_response) {
                ...
                const callback = (oo={}) => { this._trigger_remote_callback(sock, callback_id, oo); };
    ...
    _trigger_remote_callback(sock, id, o) {
        ...
        if ( sock._binary_mode.tx ) {
            sock.write(sock.object_buffer.obj2buf(
    ...
    _send_message(sock, msg) {
        ...
        if ( sock._binary_mode.tx ) {
            if ( msg.id ) {
                return new Promise(res => sock.write(sock.object_buffer.obj2buf(
