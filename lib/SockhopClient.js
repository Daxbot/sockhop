...
class SockhopClient extends EventEmitter{
            ...
            .on("data", (buf)=>{
                ...
                if ( this._binary_mode.rx ) {
                    this._object_buffer.buf2obj(buf, { return_buffers: this.debug }).forEach(({ name, obj, buffer })=>{
                        if ( this.debug ) this.emit("debug:received", obj, buffer);
            ...
            setTimeout(()=>{
                ...
                let callback=async()=>{
                    ...
                    await new Promise((resolve,reject) => {
                        ...
                        // Send handshake
                        this.send(new SockhopHandshake(),(response)=>{
                            if ( timed_out ) return;
    ...
    _trigger_remote_callback(id, o) {
        ...
        if ( this.debug ) this.emit("debug:sending", m, buff);
    ...
    _on_recieved_object(type, data, { callback_response, callback_request, callback_id }={}) {
        ...
        case "SockhopHandshake":
            ...
            this._trigger_remote_callback(callback_id, { /* TODO */});
            break;
    ...
    _send_message(msg) {
        ...
        if ( this.debug ) this.emit("debug:sending", msg, buff);
