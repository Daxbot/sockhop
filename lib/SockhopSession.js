 ...
 *
 * NOTE : This event is only emitted if the SockhopSession is in debug mode
 * @event SockhopSession#debug:sending:buffer
 * @param {object} object the object we are sending
 ...
 *
 * NOTE : This event is only emitted if the SockhopSession is in debug mode
 * @event SockhopSession#debug:received:buffer
 * @param {object} object the object we just received
 ...
 *       type set to "Buffer".
 *
 * @event SockhopSession#receive:buffer
 ...
 * @fires SockhopSession#receive:buffer
...
class SockhopSession extends EventEmitter {
    ...
    _trigger_receive_emit(obj, meta, is_typed_buffer=false) {
        this.emit("receive"+(is_typed_buffer?":buffer":""), obj, meta);
    }


    _trigger_debug_received_emit(obj, buffer, binary_mode, is_typed_buffer=false) {
        this.emit("debug:received"+(is_typed_buffer?":buffer":""), obj, buffer, binary_mode);
    }

    _trigger_debug_sending_emit(obj, buffer, binary_mode, is_typed_buffer=false) {
        this.emit("debug:sending"+(is_typed_buffer?":buffer":""), obj, buffer, binary_mode);
    }
    ...
    async send_typed_buffer(type, buff, callback){
        return this.server.send_typed_buffer(this.sock, type, buff, callback);
    }
    ...
    async send(obj, callback) {
        return this.server.send(this.sock, obj, callback);
    }
