 ...
 *       type set to "Buffer".
 *
 * @event SockhopServer#receive:buffer
 ...
 *
 * NOTE : This event is only emitted if the SockhopServer is in debug mode
 * @event SockhopServer#debug:sending:buffer
 * @param {object} object the object we are sending
 ...
 *
 * NOTE : This event is only emitted if the SockhopServer is in debug mode
 * @event SockhopServer#debug:received:buffer
 * @param {object} object the object we just received
 ...
 * @fires SockhopServer#receive:buffer
 * @fires SockhopServer#handshake
 * @fires SockhopServer#debug:sending
 * @fires SockhopServer#debug:sending:buffer
 * @fires SockhopServer#debug:received
 * @fires SockhopServer#debug:received:buffer
 * @fires SockhopError
 */
class SockhopServer extends EventEmitter {
    ...
    constructor(opts={}){
        ...
        this._allow_unsafe_encoding = !!opts.allow_unsafe_encoding;
            ...
            sock.on("end",()=>{
                ...
                if ( sock.binary_mode.rx ) {
                    sock.object_buffer.buf2obj(buf, { return_buffers: this.debug }).forEach(({ name, obj, buffer })=>{
                        ...
                        if ( this.debug ) {
                            this.emit("debug:received"+(is_typed_buffer?":buffer":""), obj, buffer, sock.binary_mode.rx, sock, sess);
                            sess._trigger_debug_received_emit(obj, buffer, sock.binary_mode.rx, is_typed_buffer);
                        }
     ...
     */
    _on_recieved_object(type, data, { callback_response, callback_request, callback_id, is_typed_buffer }={}, sock, sess) {
        ...
        default:
            ...
            if(callback_response) {
                ...
                this.emit("receive"+(is_typed_buffer?":buffer":""), data, {
                    type : type,
                    socket : sock,
                    session : sess,
                    callback,
                });
                sess._trigger_receive_emit(
                    data,
                    {
                        type : type,
                        callback,
     ...
     * @throws {SockhopError}
     */
    async send_typed_buffer(sock, type, buff, callback){
        ...
        return this._send_message(sock, m, true);
    }
     ...
     * @throws {SockhopError}
     */
    async _send_message(sock, msg, is_typed_buffer=false){
        if(!sock) {
        ...
        if ( this.debug ) {
            this.emit("debug:sending"+(is_typed_buffer?":buffer":""), msg, buff, sock.binary_mode.tx, sock, sock._session);
            sock._session._trigger_debug_sending_emit(msg, buff, sock.binary_mode.tx, is_typed_buffer);
        }
