 ...
 class ObjectBuffer extends EventEmitter{
     ...
     * @param {boolean} [opts.allow_non_objects=false] allow non objects in buf2obj (will be passed through as Strings)
     ...
     buf2obj(buf) {
         ...
         var start=0;
         var end=this._buffer.indexOf(this._rx_terminator);
         while(end>-1) {
         ...
         if(start==this._buffer.length){
             ...
             this.emit(new Error("Unexplained buffer error"));
         }
     ...
     obj2buf(obj){
         ...
         if(this._allow_non_objects===false && typeof(obj)!="object") {
             ...
             this.emit("error", new Error("obj2buf called with non object type (set allow_non_objects if you want this behavior)"));
             return;
         }
