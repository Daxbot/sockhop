 ...
 */
 class ObjectBuffer extends EventEmitter{
 	...
 	buf2obj(buf) {
 		...
 		// Find any sequences ending in '\n' (0x0a) and convert them to objects
 		var start=0;
	 	var end=this._buffer.indexOf(0x0a);
	 	while(end>-1) {
			...
			// Start is now the next character after the newline
			start=end+1; 		

			// Look for another end
		 	end=this._buffer.indexOf(0x0a,start);
	 	}
