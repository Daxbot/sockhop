...
class SockhopServer extends EventEmitter {
    ...
    constructor(opts={}){
        ...
        this.server.on('connection', async (sock) => {
            ...
            sock.on("end",()=>{
                ...
                if ( sock._binary_mode.rx ) {
                    sock.object_buffer.buf2obj(buf, { return_buffers: this.debug }).forEach(({ name, obj, buffer })=>{
                        if ( this.debug ) {
                            this.emit("debug:received", obj, buffer, sock._binary_mode.rx, sock, sess);
                            sess._trigger_debug_received_emit(obj, buffer, sock._binary_mode.rx);
                        }
                    ...
                    sock.json_object_buffer.buf2obj(buf, { return_buffers: this.debug }).forEach(({value:o, buffer})=>{
                        if ( this.debug ) {
                            this.emit("debug:received", o, buffer, sock._binary_mode.rx, sock, sess);
                            sess._trigger_debug_received_emit(o, buffer, sock._binary_mode.rx);
                        }
    ...
    _on_recieved_object(type, data, { callback_response, callback_request, callback_id }={}, sock, sess) {
        ...
        case "SockhopHandshake":
            ...
            this._trigger_remote_callback(sock, sess, callback_id, { /* TODO */ });
            break;
        case "SockhopPing":
    ...
    _trigger_remote_callback(sock, sess, id, o) {
        ...
        if ( this.debug ) {
            this.emit("debug:sending", m, buff, sock._binary_mode.tx, sock, sess);
            sess._trigger_debug_sending_emit(m, buff, sock._binary_mode.tx);
        }
    ...
    _send_message(sock, msg) {
        ...
        if ( this.debug ) {
            this.emit("debug:sending", msg, buff, sock._binary_mode.tx, sock, sock._session);
            sock._session._trigger_debug_sending_emit(msg, buff, sock._binary_mode.tx);
        }
